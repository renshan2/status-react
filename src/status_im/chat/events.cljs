...
(handlers/register-handler-fx
  ...
  (fn [{:keys [get-stored-message]} [mnemonic signing-phrase]]
    (let [crazy-math-message? (get-stored-message chat.constants/crazy-math-message-id)
...
(handlers/register-handler-fx
  ...
  (fn [{:keys [get-stored-message]} _]
    (when-not (get-stored-message chat.constants/passphrase-message-id)
...
(handlers/register-handler-fx
  ...
  [(re-frame/inject-cofx :get-stored-message)]
  (fn [{:keys [get-stored-message]} _]
    (when-not (get-stored-message chat.constants/move-to-internal-failure-message-id)
